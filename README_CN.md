...
Docker Compose 是 CliRelay 推荐的安装方式。仓库内的 `docker-compose.yml` 默认使用已发布的 `ghcr.io/kittors/clirelay:latest` 镜像，仅启动 API 服务。
...
如需本地开发时直接跑 `go run`，可使用仓库内 `Makefile` 目标：
...
- `make dev-up`: 后台启动服务端和面板 Vite 开发服务（后端为 `go run ./cmd/server`，前端在 5173）；后端通过 `MANAGEMENT_DEV_URL` 反代 `/manage` 到 Vite，面板支持热更新
- `make dev-restart`: 先停止再启动
- `make dev-down`: 停止 `dev-up` 启动的进程
...
- Web 面板：`http://localhost:8317/manage`（开发时由 Vite 提供，代理自后端），也可直接访问 `http://localhost:5173/manage`
...
- 服务端通过仓库内嵌入的 `panel/` 产物（`panel/dist`）托管管理面板，后端与面板同版本发布。
